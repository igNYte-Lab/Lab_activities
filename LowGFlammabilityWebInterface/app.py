...
def load_data(path: str):
    ...
    df[PRESSURE] = raw[SRC["pressure"]].map(parse_pressure)
    ...
    df[FSR] = pd.to_numeric(raw[SRC["fsr"]], errors="coerce")   # ratios like "Vf0G/Vf1G" are dropped
...
try:
    ...
    st.error(f"The database file could not be loaded ({exc}). "
             f"Check that `{DATA_FILE.name}` sits next to `app.py`.")
    st.stop()
...
with tab_data:
    ...
    b.caption("Units: O₂ in % by volume · pressure in kPa (converted from atm, psia, MPa and hPa "
              "where needed) · flow velocity in mm/s, negative for opposed flow · gravity in "
              "multiples of Earth gravity (g₀).")
